...
This degree project studies describes sentiment analysis with regard to mining and summarizing opinions in reviews, and provides a detailed study of some of the problems in this process.

It is layed out the following way: First there is a common introductory section with shared background for all of the three
...
Sentiment classification is the problem of deciding whether sentiment in a sentence is positive, negative or neutral\cite{nlp_book}. Being a central part of sentiment analysis it is too done on the same three levels: document-, sentence- and aspect level.
...
\subsection{\ngram~modeling}
If the probability of a word's occurrence in a sentence is known (i.e. thanks to a corpus), then the probability of a sentence $s$ can be modeled the following way based on the Markov assumption (that the probability of a word's occurrence only depend on previous word(s) in the sentence):
...
Smoothing is a means of improvement for this. %The term derives from statistics, where the goal is to make anomailites even out to better estimate the underlying distribution.
