...
\begin{figure}[t]
  ...
  \caption{\textbf{Sample review.} This review expresses negative opinions of the
    atmosphere(loud music), mixed but mostly negative opinions about the food,
    and positive mentions of the service at the restaurant.
    Review was publicly available and taken from yelp.com.}
  \label{fig:overview}
\end{figure}
...
effective to motivate focusing on the other tasks.
...
With this in mind, an experiment was developed where patterns were extended to allow
...
in the other two chapters, sentiment classification and aspect grouping.
...
in reviews. Although a sentence like
\emph{``I recommend this place over Alfredo's across the street, because their pizza tastes like rubber.''}
although perfectly reasonable in a review, the system will inaccurately discredit
...
%%%%%%%%%% toss comparable opinions and NE
...
in which case all of them were used.}. %todo balancing


Two more classifiers were developed with interpolated smoothing, 

The third classifier combines the above classifiers,
\emph{interpolated} as suggested in eq. \ref{eq:interpolated_smoothing} (where
$\lambda_2=\lambda_1=0.5)$ was found optimal).
...
Figure \ref{fig:sent_bias} shows classifiers have a slight bias towards the positive class, despite the efforts of balancing.
...
\newpage
\section{Discussion}
Of the most important results shown in Figure \ref{fig:sent_pr_curve}
is the effectiveness of smoothing.
Prior to smoothing, I tried various ways of sampling \ngram~s, stemming, lemmatization, stop-lists and entropy estimation of individual \ngram s with little to very little gain.

None of the things tested for feature selection resulted in consistent performance gain
like smoothing, where even one of the most basic methods increased performance by around 
...
So far this work has introduced methodology for extracting aspects from text and classifying their sentiments, which introduces a new task: How can these aspects with known sentiments be combined into an adequate summary?
...
amount of data is effectively multiplied by the fraction of the least common class.
...
Therefore this report will evaluate unscaled data once classifiers have been verified to be reasonably
unbiased in a per class comparison.

\section{Experiment setup}
For these experiments, Yelp reviews from the \category were manually annotated
...
Next, the map built using the word relation search was queried for the category
of each word sense. The result of each query is not only which category is nearest,
but also the distance $d$ from the word sense to the category.
This was then combined into one score using the following scheme:
...
Another important thing to keep in mind about my results are that with such a small data-set, there is a risk of methods becoming too specific toward the training data. This is often discussed in statistical modeling as \emph{``overfitting'' }, but the same phenomenon can also appear with heuristics. I believe that some aspects of my data make this heuristic perform better than it would in general.
