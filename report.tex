...
Identifying the aspects of opinions is a key task as it is what enables
...
To illustrate, imagine an entity for which every other opinion is positive and the rest
negative. Without any identification of the aspects opinions are targeting, the best
conclusion would be averaging the positive with the negative,
whereas if the aspects of opinions were identified,
it would be possible to find out if something specific is the cause of many opinions
--- possibly a very useful insight!

But identifying aspect identification is not enough. Finding that the
...
clearly refferring to the same thing!
This could potentially be a problem for aspects that are described very diversely:
To ensure reliability and counteract subjectivity, aspects with few opinions are
likely to be ignored, so there is a risk valuable opinions are disregarded unless mulitple descriptions of the same aspect are
bundled.

The task introduced to address this is \emph{aspect grouping}, or in work where
groups are dynamically derived from data; \emph{aspect clustering}.
\newpage

\subsection{Levels of grouping and this work}
todo
