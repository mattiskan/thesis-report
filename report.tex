...
\numValueAspects $ aspects. %todo equation
...
\begin{table}[h]
  ...
  \begin{tabular}{| c | c | c |}
    ...
    \emph{pos} & true positive (\emph{tp}) & false positive (\emph{fp}) \\\hline
    \emph{neg} & true negative (\emph{tn}) & false negative (\emph{fn}) \\\hline
  \end{tabular}
...
In words, it is a measure of \emph{fraction of correctly classified}.
...
In classification, particularly when classifying with multiple classes,
...
\end{equation}
And is extendend to multiclass in a similar fashion to precision\cite{sokolova2009systemati}:
...
In chapter \ref{chapter:sentiment-classifiction}, confidence thresholds are used to
allow classifiers answer \texttt{UNKNOWN} for difficult cases.
This is done so that particular instance is excluded from the result set
(and thus the precision measure), and makes recall an important measure
because instances are included in the $fn$-count, and still impact recall negatively.

The \emph{confidence threshold} is a parameter which determines how low the probability
...
All introduced evaluations are dependent on labeled data. Public datasets for products exist and were examined,
but as it has been shown that sentiment classification performs poorly cross domains\cite{aue2005customizing}
this was deemed likely to apply for aspect grouping as well. Thus it was decided to annotate new data.
...
Figure \ref{fig:cat_count} shows the distribution of classes found in this annotation.
...
\begin{figure}[t]
  ...
  \end{axis}
  \end{tikzpicture}
  \caption{Number of aspects of each category found when annotating 1000 sentences.}
  \label{fig:cat_count}
\end{figure}
...
\begin{table}[b]
  ...
  \end{tabular}
  \caption{Categories with their pre-associated key words. The key words are used as starting points for the graph search in the algorithm, and were chosen by asking three independent people about words they associate with each category, out of which these words were selected by the author.}
  \label{tab:cat_words}
\end{table}

\subsubsection{The algorithm}
A breadth first search is conducted from the initial category word senses,
as defined in table \ref{tab:cat_words}.
...
Table \ref{tab:bfs_relation} lists word relations that were either included or excluded in a similar fashion as the word senses.
...
The algorithm was optimized up to 90.2\% precsion at full recall, which was the run on the evaluation data
and resulted in 72.8\% precision at 98.5\% recall.
...
\begin{figure}[h]
  ...
  \end{axis}
  \end{tikzpicture}
  \caption{Down-sampled vs. not down-sampled classifier comparison. No confidence threshold was set in this experiment. Each bar should be compared with its corresponding graphs in the other categories. As an example, the second leftmost (red) bar in each category can be seen to have more bias towards common categories than the leftmost (blue) bar in each category, which is the down-sampled version of the same classifier. Comparison with figure \ref{fig:cat_count} will confirm that this bias correlates with the distribution of classes in the dataset.}
  \label{fig:per_cat}
\end{figure}
...
The fact that the graph search comparison achieves higher results than the machine learning classifiers is
surprising, as the method is rather static and there was genuine worry that it was overfitted to the data it
was tweaked on.
...
\item using bigrams as features did not work well --- in fact only marginally better than randomly guessing,
\item 
...
\begin{itemize}
\item sentiment is accurately classified with XX\% accuracy, or YY\% at ZZ\% recall,
\item and sentiment classifiaction is shown to be almost (XX vs YY) impartial
\end{itemize}
...
Of the most important conclusions in this report is the effectiveness language modeling.
A well developed language model and good understanding of it can be used to solve all three
tasks covered in this report, and is thus worth an investment of time.

Another conclusion the importance of balancing training data: \todo



\section{Obstacles and lessons learned}


\subsection{Studying tasks individually}
The decision to study each task individually helped with isolating concerns and understanding system behavior as a whole. But, it took a bit of back-and-fourth to figure out what my exact boundaries of the system should be. Updating one taks would change its requirements for the other tasks, which meant they needed to be updated as well.

My processes was to first get a very simple prototype running with all the tasks included; and then move on to study each step in isoaltion in order to improve overall results. I do believe in defining the performance of the full system as the product of its task's performance, but I also found that it is easy to focus too much on individual tasks and stop thinking about the system as a whole. I think it would have helped my process to reevaluate the full system between each iteration on a subtask. Afterall, each improvement to a subtask should be done with an improvement of the full system in mind.
...
\subsection{The Classifier Store}
...
its source code, which reduces the training time (about 2 minutes) to the time to
...
\section{Final words}
It was my initial hope that this report only would cover one of the three tasks introduced, but in more depth. Because of the project being hosted abroad, I began background studies and preparation on my own and most of these preparations turned out to overlap with already existing solutions at my host company. Although unfortunate, it was still a great learning experience; after studying a problem on my own, I got to compare the solutions I had come up with with existing production systems and got professional feedback on my own experiments.
