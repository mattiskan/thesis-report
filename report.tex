...
\subtitle{Getting that degree would be nice before I go work full time}
...
interpreting review-like content. This is studied in a field called \emph{Sentiment Analysis}, which has been in very active research the last ten
years thanks to increases in available data and recent efforts to monetize it.

This degree project describes sentiment analysis with regard to mining and summarizing opinions in reviews, and provides a detailed study of some of the problems in this process; identifying opinions, classifying their sentiment, and categorizing what the opinion to enable summarization.

It is laid out the following way: First there is a common introductory section with shared background for all of the three \todo
...
Readers should be aware that older literature may refer to aspects as \emph{features}, which was changed with increased application of machine learning in sentiment analysis, to avoid confusion with \emph{features} in machine learning contexts.

\subsubsection{Opinion}
\emph{Liu} (2012) introduces the opinions as $(e_i,a_{ij},s_{ijkl},h_k,t_l)$-quintuples\footnote{Subscripts are used to indicate dependencies between tuple elements.}, where $e_i$ is the entity the opinion is referencing, $a_{ij}$ is the aspect of $e_i$ that is being referenced (e.g. for a restaurant its ambiance), $s_{ijkl}$ is the sentiment of the opinion, $h_k$ is the opinion holder and $t_l$ is the time of the experience the opinion is based on\cite[Chapter~2.1]{liu2012sentiment}.
...
E.g. \emph{``running''}, \emph{``ran''} and \emph{``sprinted''} are all words representing the same lemma.
...
\emph{TODO: This is the last smoothing subsection. This is where things start to get a little complicated. Rewrite better and finish this section}
\\
\\
...
Data was divided into two equal parts, one training set and one testing set, both with equal parts positive and negative reviews.
...
Figure \ref{fig:sent_pr_curve} shows how the Modified Kneser-Ney classifier has consistently better performance, and
that that the more complex bigram model slightly outperforms the unigram model when high (around 80\%) confidence thresholds are set.
...
\begin{figure}[h]
  ...
  \begin{axis}[
    ...
    \legend{\ysc, unigram, bigram}
  \end{axis}
  \end{tikzpicture}
  \caption{Perfomance of classifiers per class.}
...
One of the most imporant results shown in Figure \ref{fig:sent_pr_curve} is that feature selection is not in itself deteriming performance. The fact that the unigram model performs better than the bigram model at full recall, but that the bigram model is better at confidence $>80\%$, indicates that the unigram model generalizes better, but that the bigram model has higher confidence in results that align well with the model. This confirms predictions made in literature.

The comparison between \ysc and the other two models is somewhat unfair, since \ysc~has been trained on slightly more data and has been rigurously tested and reviewed, but it is still included to give perspective on performance of the simpler models.

Figure \ref{fig:sent_per_cat} shows all classifiers have a slight bias towards the positive class, despite down-sampling training data to be equal. This could arguably be desired since real data has an (even larger) positive bias.
