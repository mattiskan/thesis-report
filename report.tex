...
\section{Difficult data properties}
...
\label{subsec:bias}
Another problem when dealing with real world data, it is also very possbile that some classes in classification problems are more common than others. This has been shown to have consequences both in Bayesian classsifers\cite{rennie2003bias} and Support Vector Machines\cite{svm_bias}, where both have been shown to favour majority classes\cite{rennie2003bias, svm_bias}.

Let's also think about about what happens with precision measures for scewed data.
...
Figure \ref{fig:pr_curve} shows the precision recall tradeoff for the aspect cluster classifiers.
\loremipsum
\loremipsum
